package com.mingri.web.chat.chatlist.rest;

import com.mingri.core.limit.UrlLimit;
import com.mingri.model.vo.chat.chatlist.dto.CreateDTO;
import com.mingri.model.vo.chat.chatlist.dto.DeleteDTO;
import com.mingri.model.vo.chat.chatlist.dto.ReadDTO;
import com.mingri.service.chat.repo.entity.ChatListDO;
import com.mingri.model.result.Result;
import com.mingri.service.chat.service.IChatListService;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.tags.Tag;
import org.springframework.web.bind.annotation.*;
import javax.annotation.Resource;
import javax.validation.Valid;
import java.util.List;



@Tag(name = "消息列表接口")
@RestController
@RequestMapping("/api/v1/chat-list")
public class ChatListController {

    @Resource
    IChatListService chatListService;


    @UrlLimit
    @Operation(summary = "获取私聊列表")
    @GetMapping("/list/private")
    public Object privateList() {
        List<ChatListDO> result = chatListService.privateList();
        return Result.success(result);
    }

    @UrlLimit
    @Operation(summary = "获取群聊列表")
    @GetMapping("/group")
    public Object group() {
        ChatListDO result = chatListService.getGroup();
        return Result.success(result);
    }

    @UrlLimit
    @Operation(summary = "建立私聊对话")
    @PostMapping("/create")
    public Object create(@RequestBody @Valid CreateDTO createDTO) {
        ChatListDO result = chatListService.create(createDTO.getTargetId());
        return Result.success(result);
    }

    @UrlLimit
    @Operation(summary = "标记消息已读")
    @PostMapping("/read")
    public Object read(@RequestBody @Valid ReadDTO readDTO) {
        boolean result = chatListService.read(readDTO.getTargetId());
        return Result.success(result);
    }

    @UrlLimit
    @Operation(summary = "删除私聊对话")
    @PostMapping("/delete")
    public Object delete(@RequestBody @Valid DeleteDTO deleteDTO) {
        boolean result = chatListService.delete(deleteDTO.getChatListId());
        return Result.success(result);
    }

    @Operation(summary = "新增群聊")
    @PostMapping("/group/create")
    public Object createGroup(@RequestBody @Valid CreateDTO createDTO) {
        ChatListDO result = chatListService.create(createDTO.getTargetId());
        return Result.success(result);
    }

}
