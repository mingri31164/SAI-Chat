...
package com.sai.chat.agent.infra.chat;
...
import com.sai.chat.agent.framework.trace.RagTraceNode;
...
public class BaiLianChatClient implements ChatClient {
    ...
    private JsonArray buildMessages(ChatRequest request) {
        ...
        List<com.sai.chat.agent.framework.convention.ChatMessage> messages = request.getMessages();
        if (CollUtil.isNotEmpty(messages)) {
            for (com.sai.chat.agent.framework.convention.ChatMessage m : messages) {
    ...
    private String toOpenAiRole(com.sai.chat.agent.framework.convention.ChatMessage.Role role) {
        return switch (role) {
            case SYSTEM -> "system";
            case USER -> "user";
